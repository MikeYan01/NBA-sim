...
您可以多次模拟赛季，以预测每支球队的夺冠概率。此模式运行速度更快，因为它会抑制详细的比赛输出。

- **运行 100 次模拟（默认）**：`./run.sh predict`
- **运行 N 次模拟**：`./run.sh predict N`（例如：`./run.sh predict 1000`）
- **英文模式**：`./run.sh en predict`

结果将保存到 `output/championship_prediction.txt`。
