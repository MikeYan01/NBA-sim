...
├── src    
  ...
  ├── Comments.java    解说生成模块
  ├── Constants.java    程序中大部分常量的定义文件
...
2. 判断双方球队是否需要换人
    - 常规情况下，第一节和第三节比赛进行至后后半段时，双方替补球员会换下首发球员；次节首发球员再被换回
    - 如果第四节双方球员分差过大，双方会派上替补球员打完比赛剩余时间；但如果在垃圾时间分差被快速缩小，首发球员仍然会重新登场
    - 加时赛除非是球员受伤或犯满离场，否则将由首发球员打满
...
4. 判断进攻方的出手距离，主要是根据5种球员类型判定可能的出手距离与对应概率:
    设定球员的正常出手距离是1～32英尺，在[1, 32]之间生成随机数作为出手距离。出手距离 >= 23英尺视为三分球出手。
    ...
    - 内线+三分球员，基本不投中投，因此将范围分为1～12英尺、13～22英尺、23～32英尺三段，生成的随机数只有小概率出自13～22英尺
    - 外线射手，大概率进行三分或长中投出手

    因为生成的随机数理论上在1～32的区间内是均匀分布的，但实际上大多数三分出手都是在三分线附近完成，因此本作对于较远的三分出手进行了优化，有概率将距离校正到离三分线更近的位置。
